 ...
 *   MAX_PER_CYCLE     = maksimal akun diproses per siklus (default 4)
 */
...
const MAX_PER_CYCLE = Number(process.env.MAX_PER_CYCLE || 4);
const USER_AGENT =
...
async function runCycle(reason = "timer") {
  ...
  try {
    ...
    const now = Date.now();
    const queue = (rows || [])
      .filter((r) => (cooldown.get(r.id) || 0) <= now)
      .slice(0, MAX_PER_CYCLE);
    log(`siklus ${reason}: ${rows?.length || 0} butuh refresh, proses ${queue.length}`);
