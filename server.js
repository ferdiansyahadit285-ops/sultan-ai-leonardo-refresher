/**
 * Sultan AI — Leonardo Auto Refresher (VPS / Railway) — v4.2.0
 *
...
// Chromium + checkpoint dapat memakan >1 menit per akun. Batasi jumlah akun
// per siklus supaya status `running` tidak terkunci berjam-jam.
const MAX_ACCOUNTS_PER_CYCLE = Math.max(1, Number(process.env.MAX_ACCOUNTS_PER_CYCLE || 3));
...
async function runCycle(reason = "timer") {
  ...
  try {
    ...
    const queue = eligible.slice(0, MAX_ACCOUNTS_PER_CYCLE);
