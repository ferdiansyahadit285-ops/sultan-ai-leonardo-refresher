...
const MAX_ACCOUNTS_PER_CYCLE = Math.max(1, Number(process.env.MAX_ACCOUNTS_PER_CYCLE || 6));
// Akun dianggap mendesak bila sisa umur token di bawah ini.
const URGENT_WINDOW_MS = Math.max(60000, Number(process.env.URGENT_WINDOW_MS || 20 * 60 * 1000));
...
// Satu Chromium pada Railway jauh lebih stabil. Naikkan hanya bila RAM >= 2 GB.
...
async function runCycle(reason = "timer") {
  ...
  try {
    ...
    const eligible = requestedIds.length
      ? sorted
      : sorted.filter((r) => (cooldown.get(r.id) || 0) <= now);
    // Akun mendesak tidak dibatasi jumlahnya; hanya akun santai yang dijatah.
    const urgent = eligible.filter((r) => isUrgent(r, now));
    const relaxed = eligible
      .filter((r) => !isUrgent(r, now))
      .slice(0, Math.min(MAX_ACCOUNTS_PER_CYCLE, MAX_PER_CYCLE));
    const queue = [...urgent, ...relaxed];
  ...
  setInterval(async () => {
    ...
    try {
      ...
      const urgent = (rows || []).filter(
        (r) => isUrgent(r, now) && (cooldown.get(r.id) || 0) <= now,
      );
      if (urgent.length) {
